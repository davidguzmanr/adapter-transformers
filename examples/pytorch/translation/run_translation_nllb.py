...
def main():
    ...
    # What are these for nllb?
    print(f"tokenizer.src_lang {tokenizer.src_lang}")
    print(f"tokenizer.tgt_lang {tokenizer.tgt_lang}")

    # Get the language codes for input/target. nllb model uses ISO 639-3, but the dataset uses ISO 639-1	
    source_lang = data_args.source_lang.split("_")[0][0:2]
    target_lang = data_args.target_lang.split("_")[0][0:2]
